...
const SearchResultsPage = () => {
  ...
  const { handleAddFriend, handleRemoveFriend, handleBlockUser, handleUnblockUser } = useUserActions();

  const navigate = useNavigate();

  useEffect(() => {
    if (results) {
      setSearchResultsUser(results.users || []); // Ensure searchResultsUser is an array
      setSearchResultsPost(results.posts || []); // Ensure searchResultsPost is an array
    }
  }, [results]);
  ...
  return (
    <div>
      <div className="user-list">
        <h1>Users</h1>
        {searchResultsUser && searchResultsUser.map((user, index) => (
      ...
      <div>
        <h1>Posts</h1>
        {searchResultsPost && searchResultsPost.map((post, index) => (
          <div key={index} style={{
