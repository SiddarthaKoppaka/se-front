...
export const UserActionsProvider = ({ children }) => {
  ...
  // HANDLE SEARCH
  const handleSearch = useCallback(async (searchQuery, filters = {}) => { 
      if (!token) {
        logout();
        return;
      }
  ...
  const createPage = async (pageName, description) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/pages/create`, {
        method: 'POST', 
        headers: { 
          'Authorization': `Bearer ${token}`,
  ...
  const fetchPage = async (pageId) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/pages/showPage/${pageId}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
        }
      ...
      if (response.status !== 200) {
        ...
        throw new Error('Failed to get the page');
      }
  ...
  // getAllPostsForPage
  const getAllPostsForPage = async (pageId) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/pages/getAllPostsForPage/${pageId}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
        }
  ...
  return (
    <UserActionsContext.Provider value={{ handleUpload, handleDeletePost, handleLikePost, handleUnlikePost, handleSearch,handleAddFriend, handleAcceptRequest, 
      handleRemoveFriend, handleRejectRequest, handleBlockUser, handleUnblockUser, createPage, fetchPage, getAllPostsForPage }}>
      {children}
